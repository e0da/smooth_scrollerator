...
function init() {
    ...
    options.forEach(function(key, i) {
        if (/keyboardsupport|middlemouse|pulseAlgorithm|fixedback|smoothscrollapp.marketing/.test(key)) {
...
function attachButtonListeners() {
  btns = 'custom default iphone opera ie9'.split(' ');
  for (var i = 0; i < btns.length; i += 1) {
    document.querySelector('#button_'+btns[i]).addEventListener('click', function() {
      console.log('fuck');
      set_profile(btns[i]);
    });
  }
